 ...
 * 查询所有用户的信息
 */
int QueryAllUsers(string curPage,string pageSize){
    try{
        /* create a statement object */
        stmt = con -> createStatement();

        /* run a query which returns exactly one result set */
        res = stmt -> executeQuery ("select uid,username,profile_photo,telephone,email,motto,last_login from user;");
        /* retrieve the data from the result set and display on stdout */
        Json::Value word_list;
        while(res->next()){
            Json::Value word;
            word["uid"] = (string)res->getString(1);
            word["username"] = (string)res->getString(2);
            word["profile_photo"] = (string)res->getString(3);
            word["telephone"] = (string)res->getString(4);
            word["email"] = (string)res->getString(5);
            word["motto"] = (string)res->getString(6);
            word["last_login"] = (string)res->getString(7);
 ...
 * @return
 */
int ResetPwd(string uid,string newPwd){
    try{
        /* create a statement object */
        stmt = con -> createStatement();

        /* run a query which returns exactly one result set */
        prep_stmt = con -> prepareStatement ("update user set pwd=(?) where uid=(?);");
        prep_stmt -> setString (1, newPwd);
        prep_stmt -> setString (2, uid);
...
int main ()
{
    ...
    if( !fi->isEmpty() && fi != (*formData).end()) {
        if("0"==**fi){//查询所有用户
            ...
            QueryAllUsers(**curPage,**pageSize);
        }else if("1"==**fi){//查询数据条数
            QueryTotalCount();
        }else if("2"==**fi){//重置密码
            form_iterator newPwd = formData.getElement("newPwd");
            form_iterator uid = formData.getElement("uid");
            ResetPwd(**uid,**newPwd);
        }
    }else{
